...
export default class SimpleBioEditorTabSkills extends React.Component {
  ...
  submit(data) {
    ...
    const entry1 = name1 && {
      name: name1,
      keywords: (bio.skills[0] ?
          bio.skills[0].keywords.splice(0, bio.skills[0].keywords.length, ...newKeywords1) : newKeywords1),
    };
    entries.push(entry1);
    const newKeywords2 = (_.compact([keywords2a, keywords2b, keywords2c]));
    const entry2 = name2 && {
      name: name2,
      keywords: (bio.skills[1] ?
          bio.skills[1].keywords.splice(0, bio.skills[1].keywords.length, ...newKeywords2) : newKeywords2),
    };
    entries.push(entry2);
